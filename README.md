...
<img src="image-1.png" width="640" alt="側面から撮影した歩行動画に骨格を重ねた解析画面">

*画面例：側面撮影の解析画面。奥側の脚は確信度が低いため薄い破線で表示される。*
...
動画を選んでも画面が黒いまま、進捗もエラーも出ない状態になりました。PCでは同じファイルが正常に動きます。

原因は `<video>` に `preload` を指定していなかったことでした。iOS Safari はこの場合に実質 `preload="none"` として扱うため `loadedmetadata` が発火せず、それを待つ `await` が無期限に止まっていました。例外ではないので、エラー表示にも残りません。

`preload="auto"` の指定と `load()` の明示的な呼び出しで解決しました。あわせて、イベント待ちの処理にタイムアウトを設け、来ないイベントで黙って停止しないようにしています。

切り分けでは、canvas のメモリ量や GPU の利用可否を先に疑いました。1フレームだけを処理する診断ページを作って順に確認したところ、いずれも正常でした。最終的には、動作する診断ページと動作しない本体の差分を突き合わせて特定しています。

### 7. 再生とコマ送りが重い

再生ループが `requestAnimationFrame` で毎秒約60回、骨格と軌跡を描き直していました。解析は毎秒24フレームなので、同じ内容を2〜3回繰り返し描いていたことになります。

表示するフレーム番号が変わったときだけ描画するようにして、iPhone での引っかかりがなくなりました。
...
- iPhone／Safari、Chrome、Brave（いずれもWebKit）
...
- 4K動画はiPhoneでの再生が重くなります。1080p以下を推奨します
